package com.two.my_libs.bottomnavigation;
...
public class NavigationView extends FrameLayout {
    ...
    private void addChildView(Model item, final int index){
        ...
        if (items.size()/2 == 2&&index==2){
             itemView=new NavigationItemView(getContext(),-15);
        }else {
            itemView=new NavigationItemView(getContext(),15);
        }
        ...
        if (items.size()/2 == 2&&index==2){
            iconSize = NavigationItemView.dp2px(getContext(),100);
            Log.d("NavigationView......","working"+String.valueOf(index));
            itemView.setTitle("");
        }else {
            iconSize = NavigationItemView.dp2px(getContext(),25);
        }
    ...
    public void setUnactiveTextColor(int unactiveTextColor) {
        if (this.unactiveTextColor != unactiveTextColor) {
            this.unactiveTextColor = unactiveTextColor;
            updateAllItemColors();
        }
    }

    private void updateAllItemColors() {
        for (int i = 0; i < itemContainer.getChildCount(); i++) {
            NavigationItemView itemView = (NavigationItemView) itemContainer.getChildAt(i);
            itemView.setActiveTextColor(activeTextColor);
            itemView.setUnactiveTextColor(unactiveTextColor);
            // Rebuild the item to apply color changes
            itemView.rebuild();
        }
        // Ensure the currently selected item reflects the new active color
        check(checkIndex);
    }
